...
using UnityEngine;
using Mirror;
public class MPWeaponsManager : MonoBehaviour
{
    ...
    void Awake(){
        ...
        weapons[3] = new RocketLauncherWeapon("RocketLauncher",0.1f,GetReleaseObject(3),1*6,1,1,52,55,0.25f, rocketPrefab, 5.0f, 5.0f);
    ...
    private void SwitchToAsset(int to){
        to  = (int)currentWeapon;
        if (NetworkClient.localPlayer == null){
            return;
        }
        // weaponAssets = NetworkClient.localPlayer.gameObject.GetComponent<MPWeaponsManager>().weaponAssets;
    ...
    public GameObject FireAutoWeaponMP(){
         if (weapons[(int)currentWeapon] is AutomaticWeapon){ 
            if (weapons[(int)currentWeapon].IsFireable()){
                audioManager.fireSFXEvent(weapons[(int)currentWeapon].Name,GetCurrentReleaseObject().transform.position);
                Instantiate(sparkPrefab, GetCurrentReleaseObject().transform.position, Quaternion.Euler(0,180f, 0));
